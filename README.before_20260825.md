...
- [x] **Step 6 — CAD-anchored Gaussians**：正式 512×512 RGB 验证、100,000 个 semantic Gaussians 初始化、冻结几何验证、300 步中性外观训练和 8 视角双模式渲染均已通过。
- [x] **Step 7 — 最小三方法比较（单场景 smoke）**：`Global`、`B_sem-2D`、`Global-DINO`、`Semantic-DINO` 均已完成训练/构建、8 视角渲染和 `view_06/view_07` held-out 评估；当前结果未显示 Semantic-DINO 全面优于 Global-DINO。
- [ ] **Step 8 — 完成 10 个 HSSD pair**：语义覆盖 gate 已通过；window-targeted cameras 和可审计的窗口组件 proxy 标签传播使 donor/train/held-out 六类均超过 1,000 像素。下一阶段是冻结 split、引入第二个 HSSD scene 并构造 cross-geometry pair。
...
### 当前 Smoke Test：实验设计与实际结果（更新于 2026-08-21）

这一节记录已经实际运行的实验，不是未来计划。当前已完成 **Step 1–7 的单场景 smoke test**：Global、B_sem-2D、Global-DINO 与 Semantic-DINO 已在相同 Gaussian、相机和预算下完成比较。当前结果证明数据、相机、几何、语义、外观训练和 held-out 评估管线可用，但没有证明 Semantic-DINO 全面优于 Global-DINO，更不能证明跨几何语义迁移假设成立。
...
3. 尚未在包含全部语义类别的 held-out 相机上验证 Semantic-DINO；
4. 尚未完成不同几何的 donor–target pair；
5. 尚未与 StyleGaussian、MaterialMVP 或 TRELLIS.2 比较。

因此当前里程碑是 **Step 6 passed; Step 7 single-scene smoke comparison completed with a negative/mixed result**，不是 **SemReg-GS method validated**。
...
- `scripts/run_step6_fixed.cmd` 已封装 `conda run -n semreg-gs-v1`、512×512 多模态重渲染、RGB 验证、300 步中性外观训练和双模式渲染；任何阶段失败都会停止，防止无效数据继续进入训练。

#### Step 6 正式 512×512 结果

- `rgb_validation.json` 为 `passed`：3 个 donor 与 8 个 target 共 11 个 RGB 视角具有 11 个不同 SHA-256；target 视角均为 512×512 有效 RGB。
- 正式中性外观训练在 CUDA 上观察到 83,113 / 100,000 个 Gaussian（83.11%），300 步将 L1 从 `0.247692` 降至 `0.000879`，最终 MSE 为 `1.33066e-6`、训练拟合 PSNR 为 `58.76 dB`。
- 8 个 target views 的 appearance/semantic 双模式渲染均成功输出；几何参数继续保持冻结。至此 Step 6 正式完成。

#### Step 7 当前结果

- `Global` 与 `B_sem-2D` 均严格只从 3 张 donor RGB/semantic 构建外观，然后在 8 张 target views 上评估。Global 得到 L1 `0.2954`、PSNR `9.96 dB`、颜色原型泄漏率 `100%`；B_sem-2D 得到 L1 `0.3015`、PSNR `9.10 dB`、泄漏率 `54.99%`。
- B_sem-2D 将代理颜色泄漏率降低约 45 个百分点，但整体 L1/PSNR 略差，说明简单语义均值改善区域归属的同时丢失了纹理与局部变化。
- donor 参考图包含 wall 292,174、floor 30,838、ceiling 68,294、door 108,653、other 281,504 个有效语义像素，但 window 为 0；因此 window 必须回退到 global token。这是相机覆盖问题，正式 pair 需要保证每个目标类别在 donor 中可见，或明确报告 missing-token fallback。
- Global-DINO 使用冻结的 `dinov2_vits14`（384 维）、相同冻结 CAD 几何和 4 层外观 MLP；前 6 个 target views 用于单场景监督 smoke training，`view_06/view_07` 被保留。70,509 个 Gaussian 可见，1000 步训练 L1 从 `0.24119` 降至 `0.05687`。
- Global-DINO 与 Semantic-DINO 均使用冻结的 `dinov2_vits14`、70,509 个训练可见 Gaussian、相同 4 层 MLP、1000 步、batch size 8192 和学习率 `1e-3`。Global-DINO 训练 L1 为 `0.24119 → 0.05687`；Semantic-DINO 为 `0.24093 → 0.05430`。Semantic-DINO 训练拟合略好，但必须以 held-out 结果判断。
- 在 `view_06/view_07` 的 192,663 个覆盖像素上，Global-DINO 为 L1 `0.14428`、PSNR `14.54 dB`、代理泄漏率 `95.55%`；Semantic-DINO 为 L1 `0.14493`、PSNR `14.22 dB`、代理泄漏率 `94.33%`。Semantic-DINO 将泄漏降低 `1.22` 个百分点，但 L1 增加约 `0.00065`、PSNR 降低 `0.32 dB`，没有形成全面优势。
- 分区上，Semantic-DINO 的 door L1 从 `0.21752` 改善到 `0.21185`，other 从 `0.13550` 改善到 `0.13327`；wall 从 `0.14116` 退化到 `0.14389`。这表明语义条件可能帮助少数区域，但当前证据不稳定。
- 同一 held-out 集上，Global 为 L1 `0.30719` / PSNR `9.94 dB` / 泄漏 `100%`，B_sem-2D 为 `0.34810` / `8.40 dB` / `60.72%`。学习式 DINO decoder 大幅改善重建误差，但高代理泄漏仍未解决。
- held-out 两个相机只评估到 wall 120,478、door 12,295、other 59,890 个像素；floor、ceiling、window 均为 0。特别是 donor 中 window patch 也为 0并回退到 global token，因此当前评估不能支持六类总体结论。
- 当前训练是同一场景的 6-view supervision，DINO decoder 可能利用位置编码记忆 target 外观；它只验证代码和条件化机制，不能作为 cross-geometry transfer 结果。

#### Step 8 相机语义覆盖审计

- `scripts/audit_view_semantic_coverage.py` 对每个 semantic PNG 做 nearest-palette 解码，统计六类有效像素，并穷举选择 3 个 donor 与 2 个 held-out 视角；默认要求每个 split 中每类合计至少 1,000 像素。
- 初次审计为 `failed`：原始 donor/train/held-out 的 window 均为 0。排查发现 `FP_GLASS` 位于可见窗面组件之后，单纯瞄准玻璃中心仍只会渲染前方 `other` 面。
- `scripts/generate_window_targeted_views.py` 现从三个空间独立的 `FP_GLASS` 实例生成 3 个 donor 和 6 个 target 候选，并从室内相机向玻璃中心做 ray cast。只有遮挡面与玻璃目标距离不超过 `0.50 m` 时，才把对应局部材质面组传播为 window；所有命中对象、polygon、原始材质、距离和接受状态都写入 `window_view_manifest.json`。
- 本场景 9 次 ray cast 均命中且通过距离 gate，命中距离为约 `0.209–0.242 m`。主要可见 proxy 材质为 `mig_323639`；厨房一个视角命中 `geometry_rs-2881.007 / rs-2881`。该传播属于显式、可审计的窗口组件规则，不应描述为原始逐面 GT。
- 最终 `coverage_audit.json` 状态为 `passed`。最佳 donor 为 `reference_00 / reference_01 / reference_window_02`；最佳 held-out 为 `view_05 / view_window_heldout_02`；其余候选组成 train。
- donor 六类像素为 wall 194,310、floor 27,015、ceiling 114,594、door 108,653、window 153,128、other 184,551。
- train 六类像素为 wall 1,610,894、floor 41,520、ceiling 385,925、door 392,964、window 468,780、other 753,759。
- held-out 六类像素为 wall 126,667、floor 19,808、ceiling 82,526、door 16,672、window 136,285、other 140,419。最低类别是 door 16,672，仍为 1,000 像素阈值的 16.67 倍。
- 新增 9 张 RGB 均通过有效性检查，具有 9 个不同 SHA-256；至此单场景相机语义覆盖 gate 完成。
- 当前 leakage 是“渲染 RGB 最近的 donor 类别均值原型与目标语义类别不一致的像素比例”，属于 smoke proxy，不等同于论文最终的 learned semantic leakage；后续还需 region LPIPS/DINO 与 multi-view consistency。

#### 下一实验

下一步是冻结 `coverage_audit.json` 中的 split，并下载第二个不同 scene ID，构造第一个 cross-geometry donor-target pair。训练时只能使用训练 pairs 的 target RGB；测试 pair 只提供 donor reference、target CAD 和相机/语义，decoder 不得看到测试 target RGB。首先完成 1 个 pair 的端到端验证，再扩展到 10 个 HSSD pairs。
