...
call conda run -n semreg-gs python scripts\freeze_cross_geometry_protocol.py --pair-manifest "%PAIR_DATA%\pair_manifest.json" --source-split "data\splits\%SOURCE_ID%_split.json" --coverage-audit "%PAIR_OUTPUT%\target_multiview\coverage_audit.json" --source-multiview "outputs\smoke\%SOURCE_ID%\multiview" --target-multiview "%PAIR_OUTPUT%\target_multiview" --output "%PAIR_DATA%\protocol_manifest.json"
...
call conda run -n semreg-gs python scripts\validate_semantic_gaussians.py --input "%GAUSSIANS%" --source-mesh "%TARGET_RAW%\scene.glb" --report "%GAUSSIANS%\validation.json" --max-distance 0.00001
if errorlevel 1 exit /b 1
