...
call conda run -n semreg-gs-v1 python scripts\build_source_only_dino.py --gaussians "%GAUSSIANS%" --donor-views "%DONORS%" --semantic-mapping "%MAPPING%" --protocol-manifest "%PROTOCOL%" --dino-repo "%DINO_REPO%" --dino-weights "%DINO_WEIGHTS%" --output "%OUTPUT%\appearance" --device cuda
...
for %%M in (global_dino semantic_dino) do (
  ...
  if errorlevel 1 exit /b 1
  call conda run -n semreg-gs-v1 python scripts\render_semantic_gaussians.py --gaussians "%GAUSSIANS%" --appearance "%OUTPUT%\appearance\%%M\appearance.npz" --views "%CONSISTENCY_VIEWS%" --semantic-mapping "%MAPPING%" --protocol-manifest "%PROTOCOL%" --protocol-split target_consistency --output "%OUTPUT%\consistency_renders\%%M" --mode both --point-radius 1 --device cuda
  if errorlevel 1 exit /b 1
)
